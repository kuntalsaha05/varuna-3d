...
import { Layers, Activity, Globe, Compass } from 'lucide-react';
...
export const StatsHUD: React.FC = () => {
  ...
  return (
    <div className="absolute bottom-6 left-4 z-10 bg-slate-900/80 backdrop-blur-md border border-slate-800/80 rounded-xl px-3.5 py-2.5 shadow-xl text-slate-300 text-[10px] space-y-1 select-none pointer-events-none hidden md:block">
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 font-mono text-[9px] text-slate-400">
        ...
        <div>Exaggeration: <span className="text-cyan-300 font-bold">{verticalExaggeration}x</span></div>
      </div>
    </div>
  );
};
